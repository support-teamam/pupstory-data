#!/usr/bin/env node
/**
 * Rebuilds data/manifest.json from data/pup-tasks.json.
 *
...
const DATA_PATH = join(ROOT, 'data', 'pup-tasks.json');
const MANIFEST_PATH = join(ROOT, 'data', 'manifest.json');

// Public Pages URL where the app fetches the data from.
const DATA_URL = 'https://support-teamam.github.io/pupstory-data/data/pup-tasks.json';
...
try {
  ...
  console.error('pup-tasks.json is not valid JSON:', e.message);
  process.exit(1);
}
const taskCount = Array.isArray(parsed.tasks) ? parsed.tasks.length : 0;
if (taskCount === 0) {
  console.error('Refusing to publish: tasks array is empty.');
  process.exit(1);
}
